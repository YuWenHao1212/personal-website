...
(() => {
  ...
  class DeckStage extends HTMLElement {
    ...
    _restoreIndex() {
      ...
      //               (and reopening the file) on the same slide.
    ...
    _applyIndex({ showOverlay = true, broadcast = true, reason = 'init' } = {}) {
      ...
      // ③ Durable standalone persistence — per-document key so sibling decks
      // (index / index-new / index-confluence) don't collide when opened as
      // file:// (which can share one origin).
      try { localStorage.setItem('deck-stage.idx:' + (location.pathname || ''), String(curr)); } catch (e) {}
      // URL sync — robust against file:// reloads that drop the hash.
      try {
        const inHost = (window !== window.top) || /[?&]_snthumb=/.test(location.search);
        let pending = false;
