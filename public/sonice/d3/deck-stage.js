...
(() => {
  ...
  class DeckStage extends HTMLElement {
    ...
    _restoreIndex() {
      // Resolve which slide to open on, in priority order:
      //   ① ?go=N   — explicit standalone override (deckgo helper; survives
      //               file:// reloads, where the hash can get dropped)
      //   ② #N hash — typed deep-link / presenter link / the engine's own
      //               replaceState during the session (host path uses this too)
      //   ③ stored  — standalone authoring persistence: localStorage (durable
      //               across reloads + tab close), then sessionStorage. file://
      //               can drop the hash on reload, so this is what keeps Cmd+R
      //               on the same slide for the SO NICE D3 build workflow.
      const inHost = (window !== window.top) || /[?&]_snthumb=/.test(location.search);
      // ① standalone explicit override
      if (!inHost) {
        const g = (location.search || '').match(/[?&]go=(\d+)/);
        if (g) {
      ...
      // ③ standalone durable fallback (hash dropped on file:// reload)
      if (!inHost) {
        const pkey = 'deck-stage.idx:' + (location.pathname || '');
        try {
        ...
        try {
          const v = sessionStorage.getItem('deck-stage.idx');
          if (v != null) {
            const n = parseInt(v, 10);
            if (n >= 0 && n < this._slides.length) { this._index = n; return; }
          }
    ...
    _applyIndex({ showOverlay = true, broadcast = true, reason = 'init' } = {}) {
      ...
      try { sessionStorage.setItem('deck-stage.idx', String(curr)); } catch (e) {}
      // ③ Durable standalone persistence — per-document key so d1/d2/d3 don't
      // collide when both are opened as file:// (which can share one origin).
      try { localStorage.setItem('deck-stage.idx:' + (location.pathname || ''), String(curr)); } catch (e) {}
      // URL sync — robust against the early init apply that stranded Cmd+R on P1.
      // Slots can arrive in chunks; an init apply can fire with curr=0 BEFORE the
      // deep-linked slide has been slotted. Writing the URL then would clobber the
      // deep-link (?go / #N) the restore couldn't reach yet, so the next (complete)
      // slotchange restores slide 1. Guard: while an init apply defaults to 0 but
      // the URL still points past the slides loaded so far, leave the URL untouched
      // and let the next slotchange honor the deep-link.
      try {
        const inHost = (window !== window.top) || /[?&]_snthumb=/.test(location.search);
        let pending = false;
        if (reason === 'init' && curr === 0) {
          const m = location.search.match(/[?&]go=(\d+)/) || location.hash.match(/^#(\d+)$/);
          const target = m ? parseInt(m[1], 10) - 1 : -1;
          if (target > 0 && target >= this._slides.length) pending = true;
        }
        if (!pending) {
          let url;
          if (inHost) {
            url = location.search + '#' + (curr + 1);
          } else {
            // standalone: keep ?go=N in the query — survives file:// reloads even
            // when the hash gets dropped or localStorage is blocked.
            const params = new URLSearchParams(location.search);
            params.set('go', String(curr + 1));
            url = '?' + params.toString() + '#' + (curr + 1);
          }
