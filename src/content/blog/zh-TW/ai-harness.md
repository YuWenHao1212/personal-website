...
而且 harness 可以疊加。OpenClaw 提供了基礎 harness，使用者還可以透過 SKILL.md 把自己的工作流程寫成 SOP，讓 AI 按照既定模式執行，而不是每次都從頭理解模糊的指令。SKILL.md 寫得越精準，AI 的執行品質就越穩定 — 這也是 harness 設計功力的體現。
...
> 如果對「用 AI 打造自己的 harness」有興趣，**10/24（六）台北第四班招生中**——6 小時帶你從零建立個人的 AI 指揮中心（FLUX 系統），不需要會寫程式。[看工作坊詳情 →](/zh-TW/workshop/)
...
我在下一篇文章裡實際試了 — [我建了一間 AI Agent 公司，第一天就想解散它](/zh-TW/blog/multi-agent-hidden-cost/)。把真實公司的管理方法搬到 AI agent 上，會發生什麼事？劇透：流程本身變成了瓶頸。
