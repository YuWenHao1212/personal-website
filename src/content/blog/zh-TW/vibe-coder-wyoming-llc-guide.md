...
唯一例外是 **ETBUS**（Effectively Connected Business in the US）— 也就是「你實體人在美國營業」（有美國辦公室、員工、長期停留）。99% 的 solo vibe coder 都不符合，所以不用擔心。
...
| 州 | 年度成本 | 備註 |
|---|---------|------|
| **Wyoming** | **$60** | **vibe coder 首選**（費用最低、無隱藏稅、社群資料最多）|
...
好消息：你不用自己研究 50 州規則。**Stripe Tax**（$0.5/transaction + 0.5%）會自動判斷該不該收、自動計算稅率、幫你報稅。99% 的 vibe coder 在起步期 / PMF 驗證期**根本不會觸發銷售稅**（單州營收離 $100K 很遠）。等某個州接近門檻，Stripe Tax 開起來就好。
...
這是 LLC 在「稅 + 州規費」這塊的總支出。其他維護項目（Registered Agent、電話卡、網域等）不在這裡，後面 §7 會展開。

**對台灣 vibe coder 來說，沒有「繳稅日」這個概念** — 你每年只有兩個 deadline：4/15 交聯邦揭露（不繳錢）、LLC 成立月最後一天交州 Annual Report（繳 $60 規費）。

選 Wyoming 就是 $60。這是所有 vibe coder 的標準答案。
...
LLC 不是夢想 — 是基礎設施。
報稅不用 CPA — 是驗收。
Vibe coder 不限於會寫 code — 是能用 agent ship 任何想法的 solo operator。

三件事串起來就是一個新的變現模型：

**Stripe 是統一水龍頭。** SaaS 訂閱、線上課程、一對一諮詢、Newsletter 付費會員、affiliate 收款 — 全部走同一個帳號。我現在的 Stripe 帳號同時在收：AI Resume Advisor 的 USD 訂閱、Tutoring 的 NT$8,000/堂 台幣付款。一個帳號打兩個市場、兩種幣別、兩種商業模式。

**LLC 是接通水管的費用。** $139 開、$185/年維持。這點錢換的不是「一張美國公司執照」，是「未來所有變現可能性一次解決」。明年想開 SaaS、開 course、收 newsletter 付費訂閱 — 全部直接接進同一套水管。

**AI agent 是規模化槓桿。** 客服、報稅、行銷文案、客戶 onboarding — 以前需要請人或外包，現在 agent 能做。

Vibe coder 2.0 = 用 AI agent 變現任何想法的 solo operator。不是未來，是我 2026 年的日常。
...
| 州 | 3 年成本 | 州稅 | 說明 |
|----|---------|------|------|
| **Wyoming** | ~$509 | 無 | 99% vibe coder 的選擇：州費便宜、隱私好、Member 不公開 |
