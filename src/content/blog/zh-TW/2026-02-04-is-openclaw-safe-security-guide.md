...
description: "2026 年 3 月更新。OpenClaw 安全嗎？一鍵被駭漏洞（已修復）、824+ 個惡意 Skill、VirusTotal 掃描上線。這篇 OpenClaw 安全設定教學帶你搞懂 2 大風險來源，做好 5 個必做的安全設定。"
pubDate: 2026-02-04
...
relatedPosts: ["personal-panopticon.md", "openclaw-tools-skills-tutorial.md", "2026-02-01-openclaw-deploy-cost-guide.md"]
faq:
  ...
  - question: "OpenClaw 會偷密碼嗎？"
    answer: "OpenClaw 本身不會，但惡意第三方 Skill 可能會。ClawHub 上累計發現 824+ 個惡意 Skill。2026 年 2 月起 ClawHub 已整合 VirusTotal 自動掃描，惡意 Skill 會被阻擋下載。建議只用官方 53 個 bundled Skills，第三方 Skill 安裝前務必審查。"
  - question: "OpenClaw 一定要跑在 VM 嗎？"
    answer: "不一定，但建議網路隔離。如果跑在主電腦上，攻擊者一進來就能拿到你所有資料。跑在雲端 VM（如 Azure、Hetzner）可以限制爆炸半徑，成本約 $4-8/月。"
...
> **2026-03 更新**>
> **漏洞方面**：上面提到的 1/29 一鍵被駭漏洞（[CVE-2026-25253](https://thehackernews.com/2026/02/clawjacked-flaw-lets-malicious-sites.html)，嚴重度 8.8/10）已在同日修復。之後陸續又有 7 個漏洞被發現，全部在 72 小時內修復——OpenClaw 團隊的修補速度值得肯定。>
> **惡意 Skill 方面**：數量從 341 個擴大至 824+，但 ClawHub 已於 2/7 [整合 VirusTotal 自動掃描](https://thehackernews.com/2026/02/openclaw-integrates-virustotal-scanning.html)——VirusTotal 是全球最大的惡意軟體掃描平台（Google 旗下），現在所有新上架的 Skill 都會自動經過掃描，判定為惡意的直接阻擋下載。>
> **但下面的 5 個防護設定仍然必要**——官方的修復和掃描是第一道牆，使用者端的設定才是最後防線。
...
你在 ClawHub 上找到一個看起來很好用的 Skill，說可以幫你自動整理筆記。裝了之後才發現，它的程式碼裡藏了一行：把你電腦裡的密碼和登入憑證傳到外部伺服器。這不是假設——ClawHub 上累計被發現的 824+ 個惡意 Skill 中，有一部分就是這樣運作的。
...
| **惡意 Skill** | ClawHub 上累計發現 824+ 個惡意 Skill，早期 335 個偷 macOS 密碼（2/7 起 VirusTotal 自動掃描已上線） |
...
> 💡 **延伸參考**：[Microsoft Security Blog](https://www.microsoft.com/en-us/security/blog/2026/02/19/running-openclaw-safely-identity-isolation-runtime-risk/) 建議「不要用主帳號跑 OpenClaw，用專用 VM 隔離」。[SlowMist 安全實踐指南](https://github.com/slowmist/openclaw-security-practice-guide)則提出三層防禦架構（pre-action / in-action / post-action），並建議「用最強模型跑有 tool 權限的 agent——弱模型的 prompt injection 防禦力差」。
---
...
> 想看完整的 25 個 Tools 清單和我的實際配置，請參考 [Tools & Skills 完整指南](/zh-TW/blog/openclaw-tools-skills-tutorial/)。
...
### 4. 第三方 Skill 安全嗎？怎麼判斷？

OpenClaw 除了官方 53 個 bundled Skills，在 ClawHub 社群還有 13,700+ 個第三方 Skills 可以安裝。聽起來很豐富，但這也是風險來源——累計有 824+ 個惡意 Skill 在 ClawHub 上被發現。

好消息是，2026 年 2 月 7 日起 [ClawHub 已整合 VirusTotal 自動掃描](https://thehackernews.com/2026/02/openclaw-integrates-virustotal-scanning.html)：所有新上架的 Skill 都會自動掃描，判定為惡意的會直接阻擋下載。但這不代表可以放心——[Snyk 的 ToxicSkills 研究](https://snyk.io/blog/toxicskills-malicious-ai-agent-skills-clawhub/)發現 36% 的 Skill 存在 prompt injection 風險，這類攻擊不一定會被防毒軟體抓到。
...
> **安全更新**：v2026.3.12 起，workspace plugin auto-load 已預設關閉（[GHSA-99qw-6mr3-36qr](https://github.com/openclaw/openclaw/security/advisories/GHSA-99qw-6mr3-36qr)）。這代表第三方 plugin 不會在你不知情的情況下自動啟用——但 bundled Skills 的自動載入行為不受影響，仍建議用白名單模式。
...
> ⚠️ **不要把 gateway port 暴露到公網**。綁定 localhost，需要遠端存取時用 [Tailscale](https://tailscale.com/) 或 SSH tunnel。這是多個安全研究者提到的[最常見錯誤配置](https://docs.openclaw.ai/gateway/security)。
我自己是把 OpenClaw 跑在雲端 VM 上（目前用 Azure，之後可能換到 Hetzner）。好處：
- 即使 OpenClaw 被攻破，攻擊者拿到的只是一台雲端機器，碰不到我的本機
- 可以隨時砍掉重建
- 成本約 $4-8/月（Hetzner CAX11 2vCPU/4GB 起）
---

## 所以 OpenClaw 到底安不安全？值得用嗎？
...
**OpenClaw 官方也在加速強化安全**（2026 年 2-3 月）：
| 更新 | 白話解釋 |
|------|---------|
| **手機配對更安全** | 配對用的 token 改成一次性的，用完就失效。以前是共用密碼，被偷了就一直有效 |
| **限制 agent 能存取的網址** | 可以設定白名單，只允許 agent 存取你指定的網站。沒在名單上的一律擋掉並記錄 |
| **瀏覽器自動化要認證** | 別人不能偷偷透過網路操控你的 OpenClaw 瀏覽器，必須先通過認證 |
| **審批防混淆** | 攻擊者不能再用長得像英文的特殊字元繞過你的 exec 審批 |
| **VirusTotal 掃描** | ClawHub 所有新上架 Skill 自動掃描，惡意的直接擋下載 |
| **Plugin auto-load 預設關閉** | 第三方 plugin 不再自動啟用，避免不知情載入惡意程式 |

官方的改進是第一道牆，但使用者端的設定才是最後防線。我自己做了這 5 個防護：
...
那 OpenClaw 帶來的生產力提升，絕對值得。我自己把它當作[整套 AI 工作流](/zh-TW/blog/personal-panopticon/)的手機入口——不在電腦前也能收 Daily Brief、跟系統互動。做好安全設定之後，用起來才真正放心。

### 下一步

5 個防護做完之後，下一步是搞懂 25 個 Tools 和 53 個 Skills 該怎麼配——開哪些、關哪些、為什麼。我把這部分寫在 [Tools & Skills 完整指南](/zh-TW/blog/openclaw-tools-skills-tutorial/) 裡，包含我自己的完整設定。
...
- [Running OpenClaw safely: identity, isolation, and runtime risk](https://www.microsoft.com/en-us/security/blog/2026/02/19/running-openclaw-safely-identity-isolation-runtime-risk/) - Microsoft Security Blog- [OpenClaw Security Practice Guide](https://github.com/slowmist/openclaw-security-practice-guide) - SlowMist- [OpenClaw Integrates VirusTotal Scanning](https://thehackernews.com/2026/02/openclaw-integrates-virustotal-scanning.html) - The Hacker News- [ToxicSkills: Malicious AI Agent Skills Supply Chain Compromise](https://snyk.io/blog/toxicskills-malicious-ai-agent-skills-clawhub/) - Snyk