...
| Type | Example |
|------|---------|
| **Misunderstanding** | Long conversation confuses context—you're talking about staging, it operates on production |
| **Hallucination** | OpenClaw says "done" but didn't actually do anything |
| **Overaction** | You say "draft an email," it sends it immediately |
| **Infinite Loop** | Burns hundreds of dollars in API costs in a day |
...
![OpenClaw Tools Risk vs Utility Decision Matrix](/images/blog/openclaw-security-guide/openclaw-tools-decision-matrix.webp)
...
|----------|--------------|
| OpenAI | Dashboard → [Usage limits](https://platform.openai.com/settings/organization/limits) |
| Anthropic | Dashboard → [Usage settings](https://console.anthropic.com/settings/limits) |
...
| Tool | Capability | Risk | Recommendation |
|------|------------|------|----------------|
| `exec` | Run system commands | Can delete files | ✅ Use, but enable approval |
| `write` | Write files | Can overwrite configs | ✅ Use, lock sensitive paths at system level (see below) |
| `browser` | Control webpages | Can fill forms | ✅ Use, keep "last mile" for yourself |
| `read` | Read files | Read-only | ✅ Safe to use |
...
|------|------------|-------|
| `~/.openclaw/` | OpenClaw working directory (workspace, sessions, media) | ❌ Don't lock (agent needs read/write) |
| `~/.ssh/` | SSH keys (used to log into remote servers) | 🔒 Lock |
| `~/.bashrc`, `~/.zshrc` | Shell startup config (runs every time you open terminal) | 🔒 Lock |
| `~/.config/gh/hosts.yml` | GitHub CLI token (has full repo access) | 🔒 Lock |
| `.env` or env variable files | API keys, bot tokens, etc. | 🔒 Lock |
...
| Skill | Capability | Risk | Recommendation |
|-------|------------|------|----------------|
| `gog` | Google Workspace | Can read emails, documents | ✅ Use, OAuth can be revoked anytime |
| `github` | Repo operations | Can delete repos | ✅ Use, be careful with auth scope |
| `1password` | Access password vault | Can get all passwords | ⚠️ Don't install unless absolutely necessary |
